...
function Cuidador() {
  ...
  const setTravelPoint = (point: string) => {
    ...
    if (!currentPoint) {
      ...
      const newDisabled = pointList.filter((p) => !travelPoints[point].includes(p));
      ...
      if (travelablePoints.includes(point)) {
        ...
        const newTravelablePoints = travelPoints[point].filter((p) => !traveledPoints.includes(p));
        setTravelablePoints(newTravelablePoints);
        setTraveledPoints([...traveledPoints, point]);
        const newDisabled = pointList.filter((p) => !newTravelablePoints.includes(p));
        setDisabled(newDisabled);
        pointList.forEach((p) => {
          const element = document.getElementById(p);
          if (element) {
  ...
  return (
    <div className="flex mx-auto place-content-center h-[100vh] w-full lg:w-[70%]">
      <div className="fondo-cuidador bg-[#e7ce9a] h-[100vh] p-5 w-full relative">
        ...
        <h1 className="status" id="status">{status}</h1>
        <button className="boton-generico" onClick={() => setTravelPoint(selectedPoint)}>
          <strong>Actualizar mi ubicacion</strong>
        </button>
        <div className="relative w-full h-full">
          <div className="map-container">
            <img className="mapa" src={mapa} alt="map" />
            <button className="custom-position-A" id="A" onClick={() => changeSelectedPoint("A")}>
              <img src={dot} width={20} height={50} alt="A" />
            </button>
            <button className="custom-position-B" id="B" onClick={() => changeSelectedPoint("B")}>
              <img src={dot} width={20} height={50} alt="B" />
            </button>
            <button className="custom-position-C" id="C" onClick={() => changeSelectedPoint("C")}>
              <img src={dot} width={20} height={50} alt="C" />
            </button>
            <button className="custom-position-D" id="D" onClick={() => changeSelectedPoint("D")}>
              <img src={dot} width={20} height={50} alt="D" />
            </button>
            <button className="custom-position-E" id="E" onClick={() => changeSelectedPoint("E")}>
              <img src={dot} width={20} height={50} alt="E" />
            </button>
            <button className="custom-position-F" id="F" onClick={() => changeSelectedPoint("F")}>
